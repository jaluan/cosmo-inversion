function MDML1_elevation_fig()

% This function plots results for all samples from Heimefrontfjella sorted 
% by elevation
...
load models/MDML/compiled_results_MDML1_zachos_acc.mat sample

%number of samples
Ns = 26;%length(sample);

%times for measuring erosion
TE1 = sample{1}.TE1; %TE2 = sample{1}.TE2; TE3 = sample{1}.TE3;
...
Is = [18 25 19 26 17 17 16 24 7 22 8 23 5 20 6 21 10 31 1 32 9 30 2 33 11 34]; %ordered after elevation above sea level and # nuclides, repeated if only BeAl
Is1 = [18 19 17 16 7 8 5 6 10 1 9 2 11]; %BeAl ID's ordered after elevation above sea level
Is2 = [25 26 17 24 22 23 20 21 31 32 30 33 34]; %BeAlNe/Cl ID's ordered after elevation above sea level, BeAl repeated if only BeAl exist
...
% text(-0.85,4.9,'0','rotation',90,'fontsize',7); %permil fix, not perfect
if pops == 4
    caxis([0,cmax]);
    cb1 = colorbar;
...
text(.2,1.3,'Quaternary','Interpreter','latex','Fontsize',12,'Color',0.5*[1 1 1]);
line([0,(Ns+1)],[5.333 5.333],'LineStyle','--','Color',0.5*[1 1 1],'LineWidth',.75) %Pliocene
text(.2,3.4,'Pliocene','Interpreter','latex','Fontsize',12,'Color',0.5*[1 1 1]);
text(.3,6,'Miocene','Interpreter','latex','Fontsize',12,'rotation',90,'Color',0.5*[1 1 1]);
...
line([0,41],2.6*[1,1],'color','k','linewidth',1,'linestyle','--');
...
for i=1:Ns
    ...
    elseif pops == 2, mykernel(i,2*dx,3,5,Nkernel/5,0.04,sample{snr}.d18O,col1);
    elseif pops == 3, myhalfviolin(i,dx,3,5,Nkernel/5,sample{snr}.d18O,col1,1); %myviolin(i,2*dx,3,5,Nkernel,0.04,sample{snr}.d18O,col(snr,:));
        if inVio==1 && ~(snr==snr2) && ~ismember(snr2,[32,33,34]),  myhalfviolin(i,dx,3,5,Nkernel/5,sample{snr2}.d18O,col2,2); 
        elseif inVio==1 && ~(snr==snr2) && ismember(snr2,[32,33,34]), myhalfviolin(i,dx,3,5,Nkernel/5,sample{snr2}.d18O,col4,2);
        end
    elseif pops == 6
        ...
        if inVio==1 && ~(snr==snr2) && ~ismember(snr2,[32,33,34])
        ...
        elseif inVio==1 && ~(snr==snr2) && ismember(snr2,[32,33,34])
            myhalfhist(i,dx,0,100,Nkernel/5,sample{snr2}.b1My,col4a,2); Nacc=length(sample{snr2}.b1My); % Be-Al-Cl
            myhalfhist(i,dx*length(sample{snr2}.b1My(sample{snr2}.residual<4))/Nacc,0,100,Nkernel/5,sample{snr2}.b1My(sample{snr2}.residual<4),col4b,2); % Be-Al-Cl
            myhalfhist(i,dx*length(sample{snr2}.b1My(sample{snr2}.residual<1))/Nacc,0,100,Nkernel/5,sample{snr2}.b1My(sample{snr2}.residual<1),col4c,2); % Be-Al-Cl
        end
    ...
    elseif pops == 3, myhalfviolin(i,dx,0,max(sample{snr}.TZ1),Nkernel,sample{snr}.TZ1,col1,1); %myviolin(i,2*dx,0,max(sample{snr}.TZ1),Nkernel,kmax,sample{snr}.TZ1,col(snr,:));
        if inVio==1 && ~(snr==snr2) && ~ismember(snr2,[32,33,34]),  myhalfviolin(i,dx,0,max(sample{snr2}.TZ1),Nkernel,sample{snr2}.TZ1,col2,2); 
        elseif inVio==1 && ~(snr==snr2) && ismember(snr2,[32,33,34]), myhalfviolin(i,dx,0,max(sample{snr2}.TZ1),Nkernel,sample{snr2}.TZ1,col4,2); 
        end %'w'    %   plot(i,sample{snr}.TZ1prc(3),'+','Color',pcol3);
    ...
    elseif pops == 6
        ...
        elseif inVio==1 && ~(snr==snr2) && ismember(snr2,[32,33,34])
            myhalfhist(i,dx,0,max(sample{snr2}.TZ1),Nkernel,sample{snr2}.TZ1,col4a,2); Nacc=length(sample{snr2}.TZ1); % Be-Al-Cl
            myhalfhist(i,dx*length(sample{snr2}.TZ1(sample{snr2}.residual<4))/Nacc,0,max(sample{snr2}.TZ1),Nkernel,sample{snr2}.TZ1(sample{snr2}.residual<4),col4b,2); % Be-Al-Cl
            myhalfhist(i,dx*length(sample{snr2}.TZ1(sample{snr2}.residual<1))/Nacc,0,max(sample{snr2}.TZ1),Nkernel,sample{snr2}.TZ1(sample{snr2}.residual<1),col4c,2); % Be-Al-Cl
        end
    ...
    elseif pops == 3, myhalfviolin(i,dx,0,5,Nkernel,sample{snr}.E1,col1,1); %myviolin(i,2*dx,0,max(sample{snr}.E1),Nkernel,kmax,sample{snr}.E1,col(snr,:));
        if inVio==1 && ~(snr==snr2) && ~ismember(snr2,[32,33,34]), myhalfviolin(i,dx,0,20,Nkernel,sample{snr2}.E1,col2,2); 
        elseif inVio==1 && ~(snr==snr2) && ismember(snr2,[32,33,34]), myhalfviolin(i,dx,0,20,Nkernel,sample{snr2}.E1,col4,2);
        end %'w'
    ...
    elseif pops == 6
        ...
        elseif inVio==1 && ~(snr==snr2) && ismember(snr2,[32,33,34])
            myhalfhist(i,dx,0,5,Nkernel,sample{snr2}.E1,col4a,2); Nacc=length(sample{snr2}.E1);% Be-Al-Cl
            myhalfhist(i,dx*length(sample{snr2}.E1(sample{snr2}.residual<4))/Nacc,0,5,Nkernel,sample{snr2}.E1(sample{snr2}.residual<4),col4b,2); % Be-Al-Cl
            myhalfhist(i,dx*length(sample{snr2}.E1(sample{snr2}.residual<1))/Nacc,0,5,Nkernel,sample{snr2}.E1(sample{snr2}.residual<1),col4c,2); % Be-Al-Cl
        end
